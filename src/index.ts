...
const MEMORY_BANK_TYPES: Record<string, MemoryBankType> = {
  ...
  web: {
    ...
    developmentGuidePath: path.join(__dirname, '..', 'Web', 'developmentGuide.md')
  }
};
...
async function main() {
  ...
  try {
    const { memoryBankType } = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          { name: 'Web - For TypeScript/React/Next.js development', value: 'web' }
        ]
      }
    ]);
    ...
    if (fs.existsSync(selectedType.cursorRulesPath)) {
      ...
      throw new Error(`Source .cursorrules not found: ${selectedType.cursorRulesPath}`);
    }

    // Copy development guide to .memory-bank directory
    const developmentGuideDest = path.join(memoryBankDir, 'developmentGuide.md');
    if (fs.existsSync(selectedType.developmentGuidePath)) {
      ...
      throw new Error(`Source development guide not found: ${selectedType.developmentGuidePath}`);
    }
