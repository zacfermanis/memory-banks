...
export class CrossPlatformCompatibility {
  ...
  static getPlatformInfo(): PlatformInfo {
    return {
      ...
      npmVersion:
        process.env['npm_config_user_agent']?.split('/')[0] || undefined,
    };
  }
  ...
  static async testWindowsCompatibility(): Promise<CompatibilityResult> {
    ...
    tests['installation'] = await this.runTest(
      ...
      async () => {
        ...
        if (!isWindows) {
          Logger.debug(
            'Skipping Windows installation test on non-Windows platform'
          );
        ...
        const hasNodeInPath =
          pathEnv.includes('node') || pathEnv.includes('npm');
    ...
    // Test Windows binary
    tests['binary'] = await this.runTest(
      'Windows binary',
      async () => {
        const info = this.getPlatformInfo();
        if (info.platform !== 'win32') {
    ...
    tests['optimization'] = await this.runTest(
      'Windows optimization',
      async () => {
        const info = this.getPlatformInfo();
        if (info.platform !== 'win32') {
          Logger.debug(
            'Skipping Windows optimization test on non-Windows platform'
          );
          return true;
        }
  ...
  static async testMacOSCompatibility(): Promise<CompatibilityResult> {
    ...
    tests['installation'] = await this.runTest(
      ...
      async () => {
        ...
        if (!isMacOS) {
          Logger.debug(
            'Skipping macOS installation test on non-macOS platform'
          );
          return true;
        }
    ...
    tests['optimization'] = await this.runTest(
      ...
      async () => {
        ...
        if (info.platform !== 'darwin') {
          Logger.debug(
            'Skipping macOS optimization test on non-macOS platform'
          );
          return true;
        }
  ...
  static async testLinuxCompatibility(): Promise<CompatibilityResult> {
    ...
    tests['installation'] = await this.runTest(
      ...
      async () => {
        ...
        if (!isLinux) {
          Logger.debug(
            'Skipping Linux installation test on non-Linux platform'
          );
          return true;
        }
    ...
    tests['optimization'] = await this.runTest(
      ...
      async () => {
        ...
        if (info.platform !== 'linux') {
          Logger.debug(
            'Skipping Linux optimization test on non-Linux platform'
          );
          return true;
        }
  ...
  static async testCrossPlatformValidation(): Promise<CompatibilityResult> {
    ...
    tests['compatibility'] = await this.runTest(
    ...
    tests['platformValidation'] = await this.runTest(
      ...
      async () => {
        ...
        // Test path handling
        const testPath =
          info.platform === 'win32' ? 'C:\\test\\path' : '/test/path';
    ...
    tests['reporting'] = await this.runTest(
  ...
  static async testAllPlatforms(): Promise<CrossPlatformReport> {
    ...
    Logger.info(
      'Starting comprehensive cross-platform compatibility testing...'
    );
    ...
    Logger.info(
      `Cross-platform compatibility testing completed in ${Date.now() - startTime}ms`
    );
    Logger.info(
      `Overall compatibility: ${overall.compatible ? 'PASS' : 'FAIL'}`
    );
   ...
   */
  private static calculateOverallResults(platforms: {
    [platform: string]: CompatibilityResult;
  }) {
    let totalTests = 0;
    let totalPassed = 0;
    ...
    Object.entries(tests).forEach(([testName, test]) => {
      if (!test.passed) {
        recommendations.push(
          `${platform} ${testName}: ${test.error || 'Test failed'}`
        );
      }
    });
  ...
  private static generatePlatformWarnings(
    ...
    Object.entries(tests).forEach(([testName, test]) => {
      if (test.duration > 5000) {
        warnings.push(
          `${platform} ${testName}: Slow performance (${test.duration}ms)`
        );
      }
    });
   ...
   */
  private static generateRecommendations(platforms: {
    [platform: string]: CompatibilityResult;
  }): string[] {
    const recommendations: string[] = [];
    ...
    Object.entries(platforms).forEach(([platform, result]) => {
      if (result.summary.criticalFailed > 0) {
        recommendations.push(
          `Critical failures detected on ${platform}: ${result.summary.criticalFailed} tests failed`
        );
      }

      if (result.summary.failed > 0) {
        recommendations.push(
          `Non-critical failures on ${platform}: ${result.summary.failed} tests failed`
        );
      }
    });
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'All platforms are compatible and ready for distribution'
      );
    }
